...
exports.validatePostcodeDistricts = async(req, res) => {
    try {
        ...
        var records = await PostcodeDistrict.find()
            .where("_id")
            .in(serviceAreas)
            .exec();
...
async function checkDuplicateAndPersist(req, res) {
    ...
    if (_id) {
        ...
        res
            .status(400)
            .send({
                message: `CloudKitchen already exist with name ${req.body.name}`,
            });
    } else {
        persist(req, res);
    }
...
exports.findAll = (req, res) => {
    ...
    if (req.query.supplierIds) {
        ...
        console.log("Filtering kitchens by supplierIds: " + supplierIds);
    ...
    if (req.query.keywords) {
        console.log("Received request to filter kitchens by keywords: " + req.query.keywords);
        ...
        console.log("Filtering  kitchens by keywords: " + keywords);
        // Use $in operator to filter products by supplierIds
        ...
        .catch((error) => {
            ...
            res.status(500).send({
                message: error.message ||
                    "Some error occurred while retrieving CloudKitchens.",
            });
        });
};

// Find if CloudKitchen with a name already exist
    ...
    CloudKitchen.findById(req.params.id)
        ...
        .then((data) => {
            if (!data) {
                return res
                    .status(404)
                    .send({ message: `CloudKitchen not found with id ${req.params.id}` });
            }
            res.send(data);
        })
        .catch((err) => {
            ...
            return res
                .status(500)
                .send({
                    message: `Error while retrieving CloudKitchen with id ${req.params.id}`,
                });
        });
};
...
exports.update = (req, res) => {
    ...
    if (!req.body) {
        return res
            .status(400)
            .send({ message: "CloudKitchen body cannot be empty" });
    }
    if (req.body.type) {
    ...
    CloudKitchen.findOneAndUpdate(filter, req.body)
        ...
        .catch((err) => {
            ...
            return res
                .status(500)
                .send({
                    message: `Error updating CloudKitchen with id ${req.params.id}`,
                });
        });
};
...
exports.delete = async(req, res) => {
    var _ids = await CloudKitchen.find({ _id: req.params.id }, "_id");
...
function persist(req, res) {
    ...
    console.log(
        `Attempting to persist CloudKitchen ` + JSON.stringify(localCloudKitchen)
    );
