module.exports = (app) => {
    const controller = require('../../controller/ad/ad.js');
    const { check } = require('express-validator');

    const path = process.env.CONTEXT_PATH + '/ads';
    ...
    app.post(path,
        ...
        [
            ...
            check('category').notEmpty().isLength({ min: 3, max: 250 }).withMessage('Type is mandatory'),
            check('price').notEmpty().withMessage('Price is mandatory'),
            check('description').notEmpty().withMessage('Description is mandatory'),
            check('location').notEmpty().withMessage('Ad Location is mandatory'),
            ...
            check('adOwner.email').notEmpty().isEmail().withMessage('AdOwner Email is mandatory'),
